...
function showDashboard(state: CollectorState): void {
	...
	lines.push(
		`│ ${` 🌍 Region: ${state.region}`.padEnd(W / 3)}${`⚡ Uptime: ${uptime}`.padEnd(W / 3)}${`🗃️  DB: ${dbSize}`.padEnd(W / 3)} │`,
	)
	lines.push(`├${'─'.repeat(W)}┤`)
	...
	lines.push(
		`│ ${`${natsIcon} ${natsStatus}`.padEnd(W / 4)}${`📨 Messages: ${formatNumber(state.natsMessagesReceived)}`.padEnd(W / 4)}${`🕐 Last: ${state.lastNatsMessage ? formatTimeAgo(state.lastNatsMessage) : 'Never'}`.padEnd(W / 4 - 2)}${`⚠️  Unknown locations: ${formatNumber(state.natsUnknownLocations)}`.padEnd(W / 4 + 2)}│`,
	)
	lines.push(`└${'─'.repeat(W)}┘`)
