...
const rawMaterials = require('../constants/raw-materials.json') as Array<{
	id: string
	name: string
}>
const refinedMaterials =
	require('../constants/refined-materials.json') as Array<{
		id: string
		name: string
	}>
const artifacts = require('../constants/artifacts.json') as Array<{
	id: string
	name: string
}>
const alchemyDrops = require('../constants/alchemy-drops.json') as Array<{
	id: string
	name: string
}>
...
async function fetchHistoricalData(
	...
	for (let i = 0; i < itemBatches.length; i++) {
		...
		const result = await fetchWithRetry<AODPChartRawResponse[]>(
			url,
			progressMessage,
		)
...
export async function fetchMissingHistory(): Promise<{
	...
	console.log(
		`   This requires ${itemBatches.length} API requests (${ITEMS.length} items across ${CITIES.length} cities)`,
	)
	...
	for (let rangeIdx = 0; rangeIdx < dateRanges.length; rangeIdx++) {
		...
		if (records.length > 0) {
			...
			const datesInRange = missingDates.filter(
...
export function displayHistoryStatus(): void {
	...
	if (status.missingDates.length > 0 && status.missingDates.length <= 5) {
		...
		console.log(
			`   Missing: ${status.missingDates.slice(0, 3).join(', ')} ... and ${status.missingDates.length - 3} more`,
		)
	}
}
